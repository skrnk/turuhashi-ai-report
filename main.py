...
def post_to_notion(symbol, strategy, judge, analysis):
    """分析結果をNotionデータベースに保存"""
    if not NOTION_TOKEN or not NOTION_DB_ID:
        print("⚠️ Notion設定がないため、保存をスキップします。")
        return
    ...
    # 分析内容が長い場合、Notionの制限(2000文字)に合わせてカット
    ...
    try:
        ...
        if res.status_code == 200:
            print(f"✅ Notionに保存完了: {symbol}")
        else:
            print(f"❌ Notionエラー: {res.text}")
    except Exception as e:
        print(f"⚠️ Notion送信失敗: {e}")
...
def main():
    ...
    for _, row in final_candidates.iterrows():
        ...
        prompt = f"""
        回答の冒頭に必ず 『【分析タイプ: {strategy}】』 と記載すること。
